...
def get_by_id(order_id: int):
    order_data = read_query(
        'select id, delivery_date, delivery_address from orders where id = ?', (order_id,))

    return next((Order.from_query_result(*row) for row in order_data), None)


def all():
    data = read_query('''SELECT id, delivery_date, delivery_address, product_id
                         FROM orders AS o
                           LEFT JOIN orders_products AS op
                             ON o.id = op.order_id''')

    flattened = {}
    for id, delivery_date, delivery_address, product_id in data:
        if id not in flattened:
            flattened[id] = (id, delivery_date, delivery_address, [])
...
def create(order: Order, customer: User):
    generated_id = insert_query(
        'INSERT INTO orders(delivery_date,delivery_address) VALUES(?,?)',
