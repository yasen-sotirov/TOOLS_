"""
Move
Your task is to calculate the sum of all forward and all backwards moves in an array.
On the input you will receive the starting point, the array itself with all the values.
After that you will receive multiple lines with instructions in the format [Steps Direction Size]
separated by single space until you receive exit.

The starting position must be read from the console. Direction could be forward or backwards.
If you get out of the boundaries of the array you should enter from the other side.
The starting position is not included in the sum.
...
while True:
    ...
    if command == "exit":
        break
    steps, direction, size = [int(x) if x.isdigit() else x for x in command.split()]

    if direction == "forward":
        for _ in range(steps):
            new_position = position + size
            if position + size > len(array) - 1:
                new_position = (position + size) % len(array)

            position = new_position
            forward_counter += array[position]
        ...
        for _ in range(steps):
            ...
            position = new_position
            backward_counter += array[position]

print(f"Forward: {forward_counter}")
print(f"Backwards: {backward_counter}")
