...
# next((u for u in database), None)
# извлича следващия елемент от итеруем обект или връща стойност (None).
# for u in database - Този дел на израза създава генератор,
# който итерира през database и връща стойностите му последователно, една по една.
# Генераторите са лениви, което означава, че те не извличат всички елементи на веднъж,
# а ги връщат по заявка.
#
# next(...) - Функцията next() се използва, за да извлече следващия елемент от генератора
# (или друг итеруем обект).



"ARRAY МАСИВ"
# спестява малко памет защото Питон знае колко памет да задели според типа данни
# from array import array
# float_nums = array('f', [1.2, 3.5, 4.6])
# int_nums = array('i', [1, 3, 4])
# print(type(int_nums))