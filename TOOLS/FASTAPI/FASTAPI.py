...
КОДОВЕ
    ...
    2ХХ - success сървъра казва че може да изпълни заявката ни
        201 след успешното създаване на нов ресурс чрез POST
    ...
    4ХХ - грешно подадена заявка: Client Error. Може ресурса да го няма, може юзъра не позволено да ползва ресурса, може да е нужна аутентикация
        409 сървъра среща конфликт. Например, ако се опитате да създадете ресурс, който вече съществува, код 409 указате, че вече има конфликт със съществуващия ресурс. 
...
ЕЛЕМЕНТИ НА ЗАЯВКА  - една HTTP (Hypertext Transfer Protocol) заявка (request) към уеб сървъра се състои от няколко основни елемента:

    http://     - protokol (метод), който указва какво искате да направите с ресурса на сървъра. Най-често използваните методи са GET (за извличане на информация), POST (за изпращане на данни към сървъра), PUT (за актуализация на ресурс), и DELETE (за изтриване на ресурс).
