...
is the opposite. Eating an okay amount means the dog's current 
food portion is within a range 10% above and 10% below 
...
1. Loop over the array containing dog objects, and for each dog, 
  ...
  Formula: recommendedFood = weight ** 0.75 * 28. (The result is 
...
console.log(`Sarah dog is eating too ${dogSarah.curFood > dogSarah.recFood ? 'much' : 'less'}`);
...
// 5
console.log('Is anyone eat recommended food', dogs.some(dog => dog.recFood === dog.curFood));

// 6
const checkEatingOk = dog => dog.curFood > dog.recFood * 0.9 && dog.curFood < dog.recFood * 1.1;
console.log('is ok food:', dogs.some(checkEatingOk));

// 7
const dogEatingOk = dogs.filter(checkEatingOk)
console.log('array of all dogs eating ok', dogEatingOk);
