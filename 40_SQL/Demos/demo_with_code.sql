...
-- find the cheapest and most expensive product price. What is the average price of a product?
...
-- wrong! why we cannot select columns that are not in a group by or aggregate function
select product_name, min(list_price)
from products;
...
-- wrong! why we cannot select columns that are not in a group by or aggregate function - select id - which id is that??? what does it show??
...
select pod.id as id, p.product_name, pod.quantity, pod.unit_cost, pod.date_received
...
-- update customers with id between 30 and 36 with note that tey're contractors.
update customers
